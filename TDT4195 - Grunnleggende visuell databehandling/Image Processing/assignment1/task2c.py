...
def convolve_im(im, kernel):
    ...
    zero_padded_im = np.pad(im, npad, 'constant', constant_values=0)
    flipped_kernel = np.fliplr(np.flipud(kernel))
    
    for x in range(H):
        for y in range(W):
            for color in range(RGB):    
                section = zero_padded_im[x:x+K_x, y:y+K_y, color]
                convolution_matrix = flipped_kernel * section
                im[x, y, color] = convolution_matrix.sum()
