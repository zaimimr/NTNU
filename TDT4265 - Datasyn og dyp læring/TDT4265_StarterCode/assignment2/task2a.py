...
def pre_process_images(X: np.ndarray):
    ...
    my = np.mean(X) # mean: 33.55274553571429
    sigma = np.std(X) # standard deviation: 78.87550070784701
    
    X = (X-my)/sigma

    bias = np.ones((X.shape[0],1))
    X = np.append(X, bias, axis = 1)
    print(f"mean: {my}, standard deviation: {sigma}")
    return X
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray):
    ...
    C = 1 / N * np.sum(-(targets * np.log(outputs)))
...
class SoftmaxModel:
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray,
        ...
        dk = -(targets-outputs) 
        delta_k = 1/N*(self.activations[-2].T.dot(dk))
        self.grads[-1] = delta_k
        dj = dk

        for layer in range(1,len(self.grads)):
            sp = sigmoid_prime(self.zs[-layer-1])
            dj = sp*(dj.dot(self.ws[-layer].T))
            grad_j = (1/N)*(self.activations[-layer-2].T.dot(dj))
